<script lang="ts">
	...
	async function updateDatabaseRating(newValue: number) {
		await userContext.updateBook(book.id, { rating: newValue });
	}
	...
	async function handleDrop(e: CustomEvent<any>) {
		...
		if (acceptedFiles.length) {
			...
			await userContext.uploadBookCover(book.id, file);
		}
	}
</script>
...
{#snippet bookInfo()}
	...
	<h4 class="mt-m mb-s semi-bold">Your rating</h4>
	...
	{#if !book.finished_reading_on}
		<Button isSecondary={true} onclick={updateReadingStatus}>
			{book.started_reading_on ? 'Finish reading' : 'Start reading'}
		</Button>
	{/if}
		...
		<div class="book-info">
			...
			<div class="button-container mt-m">
				<Button isSecondary={true} onclick={toggleEditMode}
					>{isEditMode ? 'Save changes' : 'Edit book'}</Button
				>
...
<style>
	...
	:global(.book-cover .dropzone.dropzone-cover) {
		height: 100%;
		border-radius: 15px;
		display: flex;
		flex-direction: column;
