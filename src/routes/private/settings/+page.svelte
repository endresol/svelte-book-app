<script lang="ts">
	...
	let avareageRating = $derived.by(() => {
		...
		const avareageRating = Math.round(100 * (sumOfAllRatings / booksWithRating.length)) / 100;
		return avareageRating;
	});
...
<div class="settings-page">
	...
	<div class="stats-container">
		...
		<h5 class="semi-bold mt-s">Finished books</h5>
		<h3>{userContext.allBooks.filter((book) => Boolean(book.finished_reading_on)).length}</h3>
		<h5 class="semi-bold mt-s">Average rating given</h5>
		<h3>{avareageRating}</h3>
	</div>
</div>
